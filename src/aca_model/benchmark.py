...
36-regime DAG with `BENCHMARK_GRID_CONFIG` grids (tiny n_points per
...
execution volume over all 36 regimes.
...
sampled from the regimes active at `start_age=51`.
"""
...
# The regimes active at start_age=51 (inelig + canwork), both marital
# statuses. All have HealthWithDisability (3-state); all but tied have
# lagged_labor_supply (tied does not — it's implied by the regime).
_INITIAL_AXES = (
...
_INITIAL_REGIMES = tuple(
    f"{marital}_{axes}" for marital in MARITAL_STATUSES for axes in _INITIAL_AXES
)
...
def get_benchmark_initial_conditions(
    *, model: Model, n_subjects: int, seed: int
    ...
    """Draw random feasible initial conditions across the age-51 regimes.
    ...
    filled with 0 (pylcm ignores them for that regime) — `spousal_income`
    included, since only the married regimes carry it.
    """
    ...
    # Grid ranges come from any initial regime (shared structure).
    ...
    tied_ids = tuple(
        model.regime_names_to_ids[f"{marital}_tied_nomc_inelig_canwork"]
        for marital in MARITAL_STATUSES
    )
    lls = np.where(
        np.isin(regime, tied_ids), 0, rng.integers(0, 2, size=n_subjects)
    ).astype(np.int32)
    ...
    return {
        ...
        "spousal_income": jnp.asarray(rng.integers(0, 2, n_subjects).astype(np.int32)),
